...
if [ $HOSTNAME = "k8s-master-1" ]; then
  ...
  sudo ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i /opt/ansible/inventory.ini cluster.yml --become -vvv  --become-user=root >> /home/vm_user_name/log.txt 2>&1
fi